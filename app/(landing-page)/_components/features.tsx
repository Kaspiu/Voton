...
const Features = () => {
  ...
  const FeatureCard = ({
    ...
    <div className="group flex flex-1 flex-col items-center gap-4 rounded-lg border bg-background p-8 text-center transition-all hover:bg-accent hover:text-accent-foreground dark:bg-input/30 dark:hover:bg-input/50">
      <div className="p-2">
        <Icon className="h-8 w-8 max-lg:h-6 max-lg:w-6" />
      </div>
      <h3 className="text-2xl font-bold max-lg:text-xl">{title}</h3>
      <p className="text-base text-muted-foreground leading-relaxed transition-all max-sm:text-sm group-hover:text-primary/70">
        {description}
      </p>
    </div>
  );
  ...
  return (
    <div id="features" className="flex w-full flex-col items-center py-24">
      <div className="flex flex-col items-center max-w-6xl gap-4 mb-12 text-center max-xl:max-w-3xl max-lg:max-w-3/4">
      ...
      <div className="flex w-full max-w-6xl gap-8 mb-8 max-xl:max-w-3xl max-lg:max-w-3/4 max-lg:flex-col">
